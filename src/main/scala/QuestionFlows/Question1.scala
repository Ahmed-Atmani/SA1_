    ...
    Flow.fromGraph(
      GraphDSL.create() {
        implicit builder =>
          ...
          val filterSunday = builder.add(Flow[Match].filter((m: Match) => m.day == "Sunday"))
          ...
          filterSunday ~> balance ~>  buffer ~> groupAndCount.async ~> merge ~> toMultiCounter ~> multiCounterReducer ~> toByteString ~> flowOut
